...
class Series: UIViewController ,InternetStatusIndicable, UIScrollViewDelegate{
    ...
    @IBAction func DismissFilter(_ sender: Any) {
        ...
        self.SeriesCollection.isUserInteractionEnabled = true
    ...
    func Filter(subcategoryId: Int?, isNewest: String?, isMostviewed: String?, from: String?, to: String?) {
            var urlComponents = URLComponents(string: "https://one-tv.net/api/episodes-filter")!
            ...
            HomeAPI.GetFiltterSearies(url: url ?? URL(fileURLWithPath: "")) { items in
                self.SeriesArray = items
                self.SeriesCollection.cr.endHeaderRefresh()
    ...
    @IBAction func ShowFillter(_ sender: Any) {
        ...
        self.SeriesCollection.isUserInteractionEnabled = false
        UIView.animate(withDuration: 0.2) {
    ...
    @IBOutlet weak var SeriesTitile: UIBarButtonItem!
    ...
    var SeriesArray: [Item] = []
    override func viewDidLoad() {
        super.viewDidLoad()
        
        self.FilterViewBottom.constant = -400
        
        SeriesTitile.setTitleTextAttributes(
                [
        ...
        SeriesCollection.register(UINib(nibName: "MovieAndSeriesCollectionCell", bundle: nil), forCellWithReuseIdentifier: "cell")
        self.SeriesCollection.delegate = self
        getCategoriesForFilter()
        getMoviews()
        getCategories()
        SeriesCollection.cr.addHeadRefresh(animator: FastAnimator(), handler: { [self] in
            getMoviews()
            getCategories()
            getCategoriesForFilter()
        })
    ...
    func getMoviews(){
        if CheckInternet.Connection(){
            HomeAPI.GetSeries { series, next in
                self.nextt = next
    ...
    func getMoviewsNext(){
        if CheckInternet.Connection(){
            HomeAPI.GetSeriesNext(self.nextt) { series, next in
                self.nextt = next
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        guard scrollView == SeriesCollection else { return }
        let pos = scrollView.contentOffset.y
        let contentHeight = self.SeriesCollection.contentSize.height
    ...
    func getbycategory(_ id: Int){
        var request = URLRequest(url: URL(string: "https://one-tv.net/api/episodes-filter?subcategoryId=\(id)")!,timeoutInterval: Double.infinity)
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
          ...
          print(String(data: data, encoding: .utf8)!)
            ...
            print(jsonData)
            let series = jsonData["data"]["episodes"]
            let free_zone = series["data"].arrayValue
...
extension Series : UICollectionViewDelegate , UICollectionViewDataSource , UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if collectionView == self.SeriesCollection{
            ...
            let urlString = self.SeriesArray[indexPath.row].image.portrait
        ...
        if UserDefaults.standard.string(forKey: "login") == "true"{
            ...
            LoginAPi.getUserInfo { info in
                if info.planId == 0{
                    ...
                    HomeAPI.GetPaidItemById(i_id: self.SeriesArray[indexPath.row].id, episode_id: 0) { items, remark, episodes, related, Astatus in
                        if Astatus == "success"{
                            ...
                            myVC.Series = items
                            myVC.title = self.SeriesArray[indexPath.row].title
                            DispatchQueue.main.async {
                                if let loadingIndicator = self.view.viewWithTag(999) as? UIActivityIndicatorView {
                                    loadingIndicator.removeFromSuperview()
